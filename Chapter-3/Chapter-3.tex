...
\section{Thermodynamic modeling of ten Ti-containing ternary systems}
...
It is worth mentioning that all the interaction parameters for the Ti-Mo-Nb-Sn-Ta-Zr system are listed in Table \ref{Ch3-table:ip} and combined into a single thermodynamic database (TDB) file in appendix a.
...
\newpage
\begin{table}[H]
	\caption{First-principles results at 0 $^{\circ}$K for the enthalpy of formation ($H_{Form}$) of the bcc phase for different mole fraction (x) of alloying element X in the Ti-X binary systems (X = Mo, Nb, Sn, Ta, Zr).}
	\centering
